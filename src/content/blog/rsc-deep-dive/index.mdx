---
author: wonhee
pubDatetime: 2024-06-15T10:16:55.000+09:00
modDatetime:
...
> "Messages MUST NOT include both a Content-Length header field and a non-identity transfer-coding. If the message does include a non-identity transfer-coding, the Content-Length MUST be ignored." - [RFC 2616](https://greenbytes.de/tech/webdav/rfc2616.html#rfc.section.4.4.p.5)
...
모던 브라우저들은 유효하지 않은 html 을 받아도, 에러를 띄우지 않고 최선을 다해서 렌더링한다.
...
RSC 서버를 최소한의 요구사항으로 간단하게 구현한 예제다.

예제에서는 페이지 첫 로드의 경우와 앱 내에서 페이지 이동이 일어난 경우를 나눠서 처리한다.
...
첫 로드는 브라우저가 기본 동작대로 보낸 해당 url에 대한 요청으로 받은 응답을 렌더링한다.
...
앱 내에서 페이지를 이동하는 경우는 기본 동작을 가로챈다.
...
- 브라우저가 url 접근하면서 SSR 서버에게 보낸 페이지 요청을 보낸다.
- 요청을 받은 SSR 서버는 페이지 전체를 렌더링해서 응답한다.

import initialResponse from "./initialResponse.js?raw";

<Code code={initialResponse} lang="js" />

- SSR 서버에서는 RSC 서버에게 컴포넌트 JSX에 해당하는 JSON 객체를 응답받는다.
- `renderToString`은 그 객체를 받아서 HTML 마크업 문자열로 반환하고, 이를 클라이언트에게 응답한다.

RSC 서버에서는 요청에 따라 해당 경로의 컴포넌트 함수를 실행해서 JSX를 반환한다.
...
서버에서 ?jsx 가 붙어서 요청이 들어오면 컴포넌트를 렌더링해서 보내줌

![invert || initial-load](./initial-load.png)

![invert || subsequent-navigation](./subsequent-navigation.png)

astro 메인테이너가 React summit 에서 발표한 예제인데, dan edition 브랜치를 보면 이 예제를 준비하면서 dan과 RSC를 스터디하 내용도 있다(유튜브 링크)

<StackBlitz id="2wheeh/simple-rsc" file="server.js" clickToLoad client:load />

클라이언트 컴포넌트, Suspense, 컴포넌트 단위 페칭을 위한 api를 적용한 버전

RSC server 를 react 내부의 `ReactDOMServer` api를 사용해서 streaming까지 지원하고,

Client Component 에 대한 처리

nextjs 의 app router는 여기에 캐싱, 최적화, 미들웨어 등 여러가지 기능을 추가한 인터페이스를 지원하는 방식인 것.
