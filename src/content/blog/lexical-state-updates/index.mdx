...
상태(_state_) 업데이트는 Lexical의 핵심 개념 중 하나이며, Lexical에서는 매우 독특한 방식으로 다뤄집니다. 이 글에선 Lexical에서 상태 업데이트가 어떻게 동작하는 지 밝혀봅니다.
...
금방 답이 나오겠지만 (스포일러: _모두 렉시컬 스코프와 관련이 있습니다_), 차근차근 알아봅시다.

# 업데이트 라이프사이클

상태를 "업데이트"나 "변경"되는 것으로 생각하면 더 쉽긴합니다. 그런 개념이 맞긴 하죠.

실제로는 에디터 인스턴스의 상태가 *업데이트*되는 게 아니라 새로운 상태로 *대체*됩니다. 에디터 상태는 기본적으로는 불변(_immutable_)하며 직접 변경(_alter_)할 수 없습니다.
...
우리 의문에 대한 답은 아니지만, 여기서 힌트는 얻을 수 있습니다: 업데이트 함수가 실행되는 동안 대기 상태는 변경할 수 있다는 것입니다.

이제 대기 상태는 변경하는 게 가능하단 걸 알았지만, _어떻게_ 변경하는지를 아직 모릅니다.
...
그런데 흥미로운 점이 있습니다. Lexical을 사용해봤으면, `LexicalEditor.update()` 콜백 **밖에서** `node.insertAfter()` 같은 메서드를 호출하려고 했을 때 다음과 같은 에러를 본 적이 있을 겁니다:
...
`insertAfter`는 읽기 전용 모드(_read-only mode_) 상태를 어떻게 아는 걸까요? `insertAfter`는 `isReadOnlyMode`과 같은 파일에서 선언되어 있지 않은데, 어떻게 동작하는 걸까요?
...
Lexical로 작업할 때, 직접 만든 함수에 이 컨벤션을 사용하는 것도 좋습니다. `$` 함수를 호출하는 함수를 만들면 `$`로 접두사를 붙인다고 생각하면 대충 맞습니다.[^4]
...
> **`$getRoot()`는 루트 노드를 어떤 에디터에서 가져올지 어떻게 아는 거죠?**
>
> `LexicalEditor.update()`가 호출될 때 에디터 인스턴스는 `activeEditor`에 저장됩니다. 이렇게 하면 `$getRoot()` 같은 함수가 (`getActiveEditor()`를 통해) 접근할 수 있게 됩니다.
...
> - "정말 멋진 글입니다." - Dominic Gannaway
> - "정말, 정말 최고에요 Dani! 정리해 줘서 고마워요! (예제 상호작용도 좋아요). 제가 보기엔 매우 정확하고 좋은 글입니다." - John Flockton
...
[^2]: Web API의 [Selection](https://developer.mozilla.org/en-US/docs/Web/API/Selection)처럼 Lexical 에디터에서 선택된 범위나 노드 등의 데이터를 위한 객체입니다 - [Lexical 공식 문서](https://lexical.dev/docs/concepts/selection)
